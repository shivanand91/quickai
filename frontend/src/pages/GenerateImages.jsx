  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        } rows={4} type="text" placeholder='Describe what you want to see in the image...' className='w-full p-2 mt-2 outline-none text-sm rounded-md border border-gray-300' required />
        ...
        <div className='my-6 flex items-center gap-2'>
          <label className='relative cursor-pointer'>
            <input type="checkbox" value={publish} onChange={(e) => setPublish(e.target.checked)} checked={publish} className='sr-only peer' />
