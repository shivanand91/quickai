...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Write an article about "${input}" with a length of ${selectedLenght.text} words.`
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <br />
        <button className='w-full flex justify-center items-center gap-2 text-sm rounded-lg cursor-pointer bg-gradient-to-r from-[#226BFF] to-[#6591ff] px-4 py-2 mt-6' disabled={loading}>
        ...
        {!content ? (
          ...
          <div className='flex-1 flex justify-center items-center'>
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              ...
              <p>Enter a topic and "Generate article" to get started</p>
            </div>
          </div>
        ) : (
